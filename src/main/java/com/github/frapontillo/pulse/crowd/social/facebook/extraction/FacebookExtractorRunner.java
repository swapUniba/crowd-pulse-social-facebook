...
public class FacebookExtractorRunner {
    ...
    private static final Logger logger = PulseLogger.getLogger(FacebookExtractorRunner.class);
    ...
    public Observable<Message> getMessages(final ExtractionParameters parameters) {
        ...
        Observable<Message> newMessages = Observable.create(new Observable.OnSubscribe<Message>() {
            @Override public void call(final Subscriber<? super Message> subscriber) {
                Observable.interval(POSTS_POLLING_MINUTES, POSTS_POLLING_MINUTES, TimeUnit.MINUTES)
        ...
        newMessages = newMessages.takeUntil(timeToWait(parameters));
        ...
        Observable<Message> mergedAndFiltered =
                Observable.merge(oldMessages, newMessages)
        ...
        mergedAndFiltered = mergedAndFiltered
                .lift(subscriber -> new SafeSubscriber<>(new Subscriber<Message>() {
    ...
    /**
     * Get the appropriate {@link rx.Observable} that will either complete after a certain amount of
     * time (depending on the input {@link ExtractionParameters#getUntil()}) or never complete.
     *
     * @param parameters The parameters, as set by the user, that can contain a null or valid "until
     *                   date".
     *
    ...
    private Observable<Long> timeToWait(ExtractionParameters parameters) {
        if (parameters.getUntil() != null) {
            ...
            logger.debug(String.format("Shutting down the Streaming service in %d seconds.",
                    timeToDeath / 1000));
    ...
    /**
     * Same as {@link FacebookExtractorRunner#getOldMessages(ExtractionParameters, rx.Subscriber,
     * boolean)} but without ever notifying the completion.
     *
     * @see FacebookExtractorRunner#getOldMessages(ExtractionParameters, rx.Subscriber, boolean)
     */
    private void getOldMessages(ExtractionParameters parameters,
            Subscriber<? super Message> subscriber) {
        getOldMessages(parameters, subscriber, true);
    }

    /**
     * Download all old messages that match the {@link ExtractionParameters}.
     *
     ...
     */
    private void getOldMessages(ExtractionParameters parameters,
            Subscriber<? super Message> subscriber, boolean complete) {
        try {
            ...
            while (query != null || page != null) {
                ...
                for (Post post : postList) {
                    ...
                    // since the Facebook API ignores "since" and "until" while paging, thus
                    // sucking big time
    ...
    private Facebook getFacebookInstance() throws FacebookException {
        return com.github.frapontillo.pulse.crowd.social.facebook.FacebookFactory
                .getFacebookInstance();
    }
     ...
     *
     * @param parameters The {@link ExtractionParameters} as specified by the user.
     *
     ...
     */
    private String handleFeedId(ExtractionParameters parameters, Facebook facebook)
    ...
    /**
     * Creates a {@link facebook4j.Reading} object from some {@link ExtractionParameters}.
     *
    ...
    private Reading buildQuery(ExtractionParameters parameters) {
        ...
        query.fields("id", "from", "to", "message_tags", "with_tags", "message", "comments",
                "created_time", "updated_time", "shares", "likes.summary(true)");
